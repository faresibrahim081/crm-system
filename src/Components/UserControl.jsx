...
function UserControl() {
    const cards = [
        ...
        { title: 'البريد الإلكتروني', content: 'ahmed@email.com', icon: <AccountCircleIcon className="w-6 h-6" /> },
        { title: 'رابط الموقع', content: 'ahmed.com', icon: <AccountCircleIcon className="w-6 h-6" /> },
        { title: 'العنوان', content: 'شارع احمد الشرقي المتقاطع من شارع احمد الشرقي...', icon: <AccountCircleIcon className="w-6 h-6" /> },
        ];
    const detail = [
        { title: 'اداره الحساب' , content: 'مكتمل', icon: <AccountCircleIcon className="w-6 h-6" /> },
        { title: ' التصميم' , content: 'قيد التنفيذ', icon: <AccountCircleIcon className="w-6 h-6" /> },
        { title: ' البرمجه' , content: 'لم يتم البدء', icon: <AccountCircleIcon className="w-6 h-6" /> },
    ]
    const Time=[
        {day :'السبت'},
        {day :'السبت'},
    ...
    return (
        <div className='UserControl bg-[#292929] text-white p-10 '>
            ...
            <div className="middle mt-8 mr-5">
                <h5 className='mb-5'>بياناتي</h5>
                <div className='data grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-4'>     
            ...
            <div className=" mt-8 mr-5 ">
                ...
                <ProjectDetails key={index} title={detail.title} content={detail.content} icon={detail.icon} />
              ))}
              </div>
            </div>
            <div className=" mt-8 mr-5 ">
  ...
  <table className="table">
    ...
    {chat.map((item) => (
            ...
            <div className=" mt-8 mr-5 ">
                ...
                <div className='data grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-4'>     
                {Time.map((card, index) => (
                <YourTime key={index} title={card.title} content={card.content} icon={card.icon} />
                ))}
                </div>
            </div>
